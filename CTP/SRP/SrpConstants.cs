...
namespace CTP.SRP
{
    ...
    internal class SrpConstants
    {
        ...
        public readonly int PaddedBytes;

        private SrpConstants(string hexN, int decG)
        {
            N = BigInteger.Parse(hexN, NumberStyles.HexNumber);
            g = new BigInteger(decG);
            PaddedBytes = N.ToUnsignedByteArray().Length;
            k = ComputeHash(N, g);
        }

        private static BigInteger ComputeHash(BigInteger N, BigInteger g)
        {
            using (var sha = SHA512.Create())
            {
                ...
                return sha.ComputeHash(hash).ToUnsignedBigInteger();
            }
        }
