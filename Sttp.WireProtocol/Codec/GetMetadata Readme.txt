...
6) If a primary key is modified, all referencing rows must have that reflected and update the row version of that row.
...
8) When deleting a row, either change the schema version so all data is invalidated, or do the same as if the primary key was changed, except a permanent list 
